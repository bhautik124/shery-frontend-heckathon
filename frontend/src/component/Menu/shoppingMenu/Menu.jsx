...
const Menu = ({ onCartClick }) => {
  ...
  return (
    <div className="flex flex-col items-center font-neuebrucke text-2xl sm:text-3xl md:text-4xl font-light text-black uppercase relative">
      {!menuOpen && (
        <div className="flex items-center gap-3 sm:gap-4 md:gap-6 bg-[#d9d9d9] p-2 sm:p-3 md:p-4 rounded-md cursor-pointer">
          <div
          ...
          <div
            className="flex items-center justify-center w-10 sm:w-12 md:w-14 h-10 sm:h-12 md:h-14 rounded-md bg-black hover:bg-white border-2 border-black transition-all duration-300 cursor-pointer"
            onClick={onCartClick}
          >
            <BsCart3 className="text-white hover:text-black text-xl sm:text-2xl md:text-3xl transition-colors duration-300" />
          </div>
        </div>
      )}
      ...
      <div
        ref={menuRef}
        className="absolute w-[90vw] sm:w-[24rem] md:w-[28rem] lg:w-[30rem] bottom-full bg-[#d9d9d9] text-black p-4 sm:p-5 md:p-6 rounded-xl text-2xl sm:text-3xl md:text-4xl hidden shadow-lg"
      >
